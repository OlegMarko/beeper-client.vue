        ...
        methods: {
            likeBeep: function () {
                if (!this.$auth.loggedIn()) {
                    alertify.error('You need to login if you want to do that!')
                    return
                }
            ...
            beepTime: (timestamp) => {
                return moment(timestamp + 1000).format('HH:ss')
            }
        }
    }
</script>
...
<style scoped>
    ...
    .beepFooter .btn-default { color: #734b6d; }
    .beepFooter .btn-primary { background: #734b6d; }
    .beepFooter strong { margin-top: 7px; }
</style>
